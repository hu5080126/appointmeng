...
const dateUtil = {
    ...
    dateDiff(val) {
        let range1 = [-Infinity, 10] // 0~10分钟
        let range2 = [10, 60] // 10分钟~1小时
        let range3 = [60, 24 * 60] // 1小时~1天
        let range4 = [24 * 60, 48 * 60] // 1天~2天
        let range5 = [48 * 60, 365 * 24 * 60] // 2天~1年
        let range6 = [365 * 24 * 60, Infinity] // 1年~
        let beforeDate = new Date()
        let currentDate = new Date()
        let diff

        if (val) {
            // beforeDate = dateUtil.getMoment(val).toDate()
            beforeDate = new Date(val)
        }
        let m = beforeDate.getTime()
        ...
        if (diff > range1[0] && diff <= range1[1]) {
            ...
            if (Math.abs(beforeDate.getDay() - currentDate.getDay()) > 1) {
                return `${dateUtil.format(m, 'M月d日 HH:mm')}`
            }
            return `昨天${dateUtil.format(m, 'HH:mm')}`
        } else if (diff > range5[0] && diff <= range5[1]) {
            return `${dateUtil.format(m, 'M月d日 HH:mm')}`
        } else if (diff > range6[0] && diff <= range6[1]) {
            return `${dateUtil.format(m, 'yyyy年M月d日 HH:mm')}`
        }
    }
};
...
export let dateDiff = dateUtil.dateDiff;
